...
/**
 * @page performance-tcppingpong Checking TCP performance
 *
 * @objective Run @b zftcppingpong to get mean @b RTT for @b TCP.
 *
 * @param env                 Testing environment:
 *                            - @ref arg_types_env_peer2peer
 * @param use_muxer           If @c TRUE, pass "-m" option to
 *                            @b zftcppingpong.
 * @param overlapped_delay    If nonzero, enable overlapped reads and
 *                            specify overlapped delay in nanoseconds
 *                            with "-c" option (makes sense only if
 *                            @p use_muxer is @c TRUE):
 *                            - @c 0
 *                            - @c 10
 *
 * @type Conformance.
...
#define TE_TEST_NAME "performance/tcppingpong"
...
/** How long to wait for zftcppingpong termination, in ms */
...
main(int argc, char *argv[])
{
    ...
    TEST_GET_INT_PARAM(overlapped_delay);
    ...
    app_opts.use_muxer = use_muxer;
    if (overlapped_delay > 0)
        app_opts.overlapped_delay = TAPI_JOB_OPT_UINT_VAL(overlapped_delay);

    TEST_STEP("Allocate structures for running @b zftcppingpong on "
              "IUT (as client) and on Tester (as server).");
    CHECK_RC(zfts_perf_create_tcp_pingpong_clnt(ping_clnt_factory,
                                                &app_opts, &ping_clnt));
    CHECK_RC(zfts_perf_create_tcp_pingpong_srv(ping_srv_factory, &app_opts,
                                               &ping_srv));

    TEST_STEP("Run @b zftcppingpong as server on Tester.");
    CHECK_RC(zfts_perf_start_app(ping_srv));
    TAPI_WAIT_NETWORK;

    TEST_STEP("Run @b zftcppingpong as client on IUT.");
    CHECK_RC(zfts_perf_start_app(ping_clnt));

    TEST_STEP("Wait for termination of @b zftcppingpong on IUT "
              "and Tester.");
    CHECK_RC(zfts_perf_wait_app(ping_srv, PINGPONG_TIMEOUT));
    CHECK_RC(zfts_perf_wait_app(ping_clnt, PINGPONG_TIMEOUT));

    TEST_STEP("Get mean @b RTT value printed out by @b zftcppingpong "
              "on IUT.");
    CHECK_RC(zfts_perf_get_single_result(ping_clnt, &mean_rtt));
    TEST_ARTIFACT("Mean RTT is %f us", mean_rtt);

    CHECK_RC(zfts_perf_mean_rtt_to_mi("zftcppingpong", mean_rtt));
