...
# Contributing code to ZetaFerno Test Suite

The document provides the guidelines for submitting changes to ZetaFerno TS.
...
The summary line should:
 ...
 - be prefixed with the component name, for example:
    ...
    - `doc`
    - `scripts`
    - `muxer|udp_rx|zf_alt|...` (for changes related to a specific package)
    - `muxer/mod_events` (for changes related to a specific test)
 ...
 - start with the imperative of a verb, for example:
   ...
   ```
   overlapped/recv_pftf: add EPOLLIN event handling
   ```

   ```
   ts: disable ZF tests which incompatible with TX timestamps
   ```
